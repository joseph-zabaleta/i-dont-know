...
def scrape_yelp_iris(item):
    ...
    with open('./assets/test-data.txt', 'w+') as f:
        ...
        # item = result[4].text.strip()  # this is w/o html
        item = str(result[4])
        f.write(f"\n{'*'*50}\n")
        f.write(item)
        f.write(f"\n{'*'*50}\n")

        sub_content = result[4]
        
        # i can't use BeautifulSoup un a sub_content, children and descendants are not returning anything
        soup_name = BeautifulSoup(sub_content, "html.parser")  
        print(soup_name.prettify())
        # result_name = soup_name.find_all('a', class_="lemon--a__373c0__IEZFH link__373c0__1G70M link-color--inherit__373c0__3dzpk link-size--inherit__373c0__1VFlE")
        # print('The restaurant name is: ', result_name.text.strip())
